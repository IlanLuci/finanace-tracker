#!/usr/bin/env python3
"""
Script to change the most recent deposit date from April 2nd to April 1st
for each portfolio in the data directory.
"""
...
def date_to_timestamp(year, month, day):
    """Convert date to Unix timestamp (at 16:00 UTC equivalent)."""
    dt = datetime(year, month, day, 16, 0, 0)
    return int(dt.timestamp())

def main():
    data_dir = 'data'
    
    # April 2 start and end timestamps (entire day)
    april_2_start = date_to_timestamp(2026, 4, 2) - (16 * 3600)  # Midnight UTC
    april_2_end = april_2_start + (24 * 3600)  # Next day midnight
    
    # April 1 reference (will preserve time of day)
    april_1_base = date_to_timestamp(2026, 4, 1) - (16 * 3600)  # Midnight UTC
    
    print(f"Looking for deposits on April 2, 2026")
    print(f"Will change to April 1, 2026 (preserving time of day)\n")
    ...
    for filepath in portfolio_files:
        ...
        # Find all deposits with date on April 2 (any time on that day)
        deposits_on_april_2 = [
            (i, tx) for i, tx in enumerate(transactions)
            if tx['type'] == 0 and april_2_start <= tx['date'] < april_2_end  # Type 0 = DEPOSIT
        ]
        
        if not deposits_on_april_2:
            print(f"  No deposits found on April 2, 2026\n")
            continue
        
        # Find the most recent one
        most_recent_idx, most_recent_tx = max(
            deposits_on_april_2,
            key=lambda x: x[1]['date']
        )
        
        print(f"  Found {len(deposits_on_april_2)} deposit(s) on April 2")
        print(f"  Changing most recent deposit: ${most_recent_tx['amount']:.2f}")
        print(f"  Original date: {timestamp_to_date(most_recent_tx['date'])}")
        
        # Calculate April 1 timestamp with same time of day
        time_of_day = most_recent_tx['date'] - april_2_start
        new_date = april_1_base + time_of_day
        
        # Change the date
        transactions[most_recent_idx]['date'] = new_date
        
        print(f"  New date:      {timestamp_to_date(new_date)}")
